...
/*条件编译参数*/
// #define GC_ROT_PACKAGE	//夹手旋转放置玻片程序宏定义
#define GC_VER_PACKAGE	  // 夹手垂直放置玻片程序宏定义
...
struct glass_claw_struct
{
	...
	u32 GCV_pacakge_pos;			// 夹手垂直下降到封片位置距离
	u8 subtask;						// 子任务
	u8 main_subtask;				// 主任务
...
struct glass_out_struct
{
	...
	u32 GOH_mid_pos;	  // 封片位置
	u32 GOH_end_pos;	  // 终点存储位置
	u32 GOV_box_dis;	  // 存储盒间距
	u32 GOV_slot_dis;	  // 存储盒槽间距
	u32 GOV_box_len;	  // 存储盒最大长度
	u16 GOV_adjust;		  // 存储盒调整距离
	u16 GOV_adjust_start; // 原点存储盒调整距离
	u8 subtask;			  // 子任务
	u8 main_subtask;	  // 主任务
	u8 Index;			  // 通讯协议序号
	u8 WaitAck;			  // 通讯协议应答标志
